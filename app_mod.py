...
math_dict_url = os.path.join(SITE_ROOT, "static", "math_dict.json")
math_dict_data = json.load(open(math_dict_url, encoding="utf8"))

# edict_url = os.path.join(SITE_ROOT, "static", "engdict.json")
# edict_data = json.load(open(edict_url, encoding="utf8"))
...
#     opposite_value = ''
#     bangla_value = "<br><div class='well text-dark'><b>Việt Nam: </b>"+bangla_dict['bangla']+"</div>"
...
def fetch_word():
    if request.method == "POST":
        ...
        english_value = ''
        if word_lower in math_dict_data:
            english_value = clean_english_value(math_dict_data[word_lower])
            if word_lower in edict_data:
                bangla_value = clean_bangla_value(edict_data[word_lower])
